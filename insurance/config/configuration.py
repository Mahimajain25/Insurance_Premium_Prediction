...
class Configuration():
    ...
    def get_data_ingestion_config(self) -> DataIngestionConfig:
        try:
            ...
            data_ingestion_config = os.path.join(artifact_dir, DATA_INGESTION_ARTIFACT_DIR, self.timestamp)

            dataset_download_url = data_ingestion_info[DATA_INGESTION_DOWNLOAD_URL_KEY]

            zip_download_data = os.path.join(data_ingestion_config,
                                             data_ingestion_info[DATA_INGESTION_ZIP_DOWNLOAD_DIR_KEY])

            raw_data_dir = os.path.join(data_ingestion_config, data_ingestion_info[DATA_INGESTION_RAW_DATA_DIR_KEY])

            ingested_train_dir = os.path.join(data_ingestion_config,
                                              data_ingestion_info[DATA_INGESTION_INGESTED_DIR_NAME_KEY],
                                              data_ingestion_info[DATA_INGESTION_TRAIN_DIR_KEY])

            ingested_test_dir = os.path.join(data_ingestion_config,
                                             data_ingestion_info[DATA_INGESTION_INGESTED_DIR_NAME_KEY],
                                             data_ingestion_info[DATA_INGESTION_TEST_DIR_KEY])

            data_ingestion_config = DataIngestionConfig(dataset_download_url=dataset_download_url,
                                                        zip_download_data=zip_download_data,
                                                        raw_data_dir=raw_data_dir,
                                                        ingested_test_dir=ingested_test_dir,
                                                        ingested_train_dir=ingested_train_dir)
            logging.info(f"Training pipleine config: {data_ingestion_config}")
    ...
    def get_data_validation_config(self) -> DataValidationConfig:
        try:
            ...
            logging.info(f"Training pipleine config: {data_validation_config}")
    ...
    def get_data_transformation_config(self) -> DataTransformationConfig:
        try:
            ...
            smoker_cat = data_transformation_config_info[DATA_TRANSFORMATION_SMOKER_CAT_KEY]
